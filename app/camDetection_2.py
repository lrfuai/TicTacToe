        ...
        if self.__haveSubt == True :
            for pos_y in vpos_y:
                for pos_x in vpos_x:
                    ...
                    diff1 = cv2.cvtColor(img_1, cv2.COLOR_BGR2GRAY)  
                    diff1 = cv2.GaussianBlur(diff1, (21, 21),0) 
                    
                    img_2 = cv2.imread('imagen//'+ sFile +'_T.png')
                    diff2 = cv2.cvtColor(img_2, cv2.COLOR_BGR2GRAY) 
                    ...
                    #imagen_gris= resta.copy()

                    canny = cv2.Canny(resta, 10, 60)
                    ...
                    for c in contornos:     
                        ...
                        if cv2.contourArea(c) < 60 and  cv2.contourArea(c) > 10  :
