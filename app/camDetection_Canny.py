...
class CamDetection :
    ...
    def process(self, key, showImage) :
        ret_val, img_color = self.__cam.read(1)
        #webcam = cv2.VideoCapture(1)
        #gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

        #rota la imagen e la invierte
        ...
        img_gris = cv2.GaussianBlur(img_gris, (25, 25),0)
        # Aplicamos un umbral
        umbral = cv2.threshold(img_gris, 100, 250, cv2.THRESH_BINARY)[1]
        # Dilatamos el umbral para tapar agujeros
        #umbral = cv2.dilate(umbral, None, iterations=2)
        if (self.__bDebuge): cv2.imshow('gris', umbral)
        ...
        if self.__updateRef == True :  #con letra s graba imagenes de referencia
            ...
            for pos_y in vpos_y:
                for pos_x in vpos_x:
                    #pinta el rectagulo en la imagen a color
                    #cv2.circle(img,(pos_x,pos_y), 80, (0,255,255), 1)
                    cv2.rectangle(img_color,(pos_x-80,pos_y-80),(pos_x+80,pos_y+80),(0,255,0),2)

                    #arma nombre del archivo de las imagenes de referencia en color gris  
        ...
        if self.__haveSubt == True :
            for pos_y in vpos_y:
                for pos_x in vpos_x:
                    ...
                    resta = cv2.absdiff(diff2, diff1)
                    #if (self.__bDebuge): cv2.imshow('resta', resta)
                    ...
                    canny = cv2.Canny(resta, 10, 150)
         
                    
         
                    # Buscamos los contornos
                    im, contornos,hierarchy = cv2.findContours(canny.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  
                    for c in contornos:     
                        if (self.__bDebuge): print (cv2.contourArea(c))
                        if cv2.contourArea(c) < 10000 and  cv2.contourArea(c) > 4000  :         
                            
                            if (self.__bDebuge): cv2.imshow("canny", canny)

                            cv2.circle(img_color,(pos_x,pos_y), 30, (0,255,255), 2)  
                    ...
                    # # Buscamos los contornos
                    # im, contornos, hierarchy = cv2.findContours(img,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
                    ...
                    #         #cv2.rectangle(img,(posicion_x,posicion_y),(posicion_x+ancho,posicion_y+alto),(0,255,0),1) #Dibujamos la bounding box sobre diff1
        ...
        if(showImage == True) :    
            cv2.imshow('img_color', img_color)  #muetra la ficha detectada
            self.__imgMatriz = img_color.copy()  #copia en la imagen global que usara pintarficha
