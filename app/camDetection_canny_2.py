        ...
        if self.__haveSubt == True :
            for pos_y in vpos_y:
                for pos_x in vpos_x:
                    ...
                    img_1 = cv2.imread('imagen//'+ sFile +'.png')  
                    gris_1 = cv2.cvtColor(img_1, cv2.COLOR_BGR2GRAY)  
                    gauss_1 = cv2.GaussianBlur(gris_1, (21,21), 0)     # Aplicar suavizado Gaussiano     

                    img_2 = cv2.imread('imagen//'+ sFile +'_T.png')
                    gris_2 = cv2.cvtColor(img_2, cv2.COLOR_BGR2GRAY)  
                    gauss_2 = cv2.GaussianBlur(gris_2, (21,21), 0)  # Aplicar suavizado Gaussiano
                    ...
                    #busca contorno por metodo canny
                    canny = cv2.Canny(resta, 10, 150)
                    # Buscamos los contornos
                    im, contornos, hierarchy = cv2.findContours(canny,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
                    # Buscamos los contornos
                    ...
                    if (len(contornos) > 0 ):
                        ...
                        #cv2.drawContours(canny,contornos,-1,(0,0,255), 2)
                    ...
                    for c in contornos:                   
                        if cv2.contourArea(c) < 25000 and  cv2.contourArea(c) > 8000  :
                            ...
                            cv2.rectangle(img_1,(posicion_x,posicion_y),(posicion_x+ancho,posicion_y+alto),(0,255,0),1) #Dibujamos la bounding box sobre diff1
